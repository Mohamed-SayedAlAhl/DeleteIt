...
namespace Delete_It // Your namespace
{
    public class ProcessKiller
    {
        ...
        private static DialogResult ShowFullProcessMessageForFolders()
        {
           ...
           DialogResult dialogResult = CustomMessageBox.Show($"{Message}\n Do you want to close all apps to continue deleting the folder?",
        ...
        private static bool IsEulaAccepted()
        {
            try
            {
                // Check in the registry where Sysinternals tools like handle.exe store EULA acceptance
                using (RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\Sysinternals\Handle"))
                {
                    if (key != null)
                    {
                        ...
                        {
                            return true; // EULA is already accepted
                        }
                    }
                }
            }
        ...
        public static condition CloseProcessUsingFolder(string FolderPath)
        {
